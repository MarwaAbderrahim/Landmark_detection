...
# #     # Définition de la taille de la sous-image
# #     sub_image_size = [100, 100, 100]  # Par exemple, une taille de sous-image de 100x100x100 voxels

# #     # Initialisation des listes pour stocker les résultats
# #     all_landmarks_pred = []

# #     # Boucle pour parcourir les sous-images
# #     for x in range(start_voxel[0], end_voxel[0], sub_image_size[0]):
# #         for y in range(start_voxel[1], end_voxel[1], sub_image_size[1]):
# #             for z in range(start_voxel[2], end_voxel[2], sub_image_size[2]):
# #                 # Définition des coordonnées de début et de fin de la sous-image
# #                 sub_start_voxel = [x, y, z]
# #                 sub_end_voxel = [min(x + sub_image_size[0], end_voxel[0]),
# #                                  min(y + sub_image_size[1], end_voxel[1]),
# #                                  min(z + sub_image_size[2], end_voxel[2])]
# #                 # Extraction de la sous-image
# #                 sub_roi_image = iso_image[sub_start_voxel[0]:sub_end_voxel[0],
# #                                            sub_start_voxel[1]:sub_end_voxel[1],
# #                                            sub_start_voxel[2]:sub_end_voxel[2]]
                
# #                 # Inférence sur la sous-image
# #                 landmarks_pred = inference_on_sub_image(model, sub_roi_image, use_gpu)
                
# #                 # Ajout des résultats à la liste
# #                 all_landmarks_pred.append(landmarks_pred)

# #     return all_landmarks_pred


# # def inference_on_sub_image(model, sub_image, use_gpu):
# #     """ Perform inference on a single sub-image
# #     :param model:       the loaded segmentation model.
# #     :param sub_image:   the sub-image to perform inference on.
# #     :param use_gpu:     whether to use gpu or not, bool type.
# #     :return:            the predicted landmarks for the sub-image
# #     """
# #     if model['crop_normalizers'] is not None:
# #         sub_image = model.crop_normalizers[0](sub_image)

# #     sub_image_tensor = convert_image_to_tensor(sub_image).unsqueeze(0)
    
# #     if use_gpu:
# #         sub_image_tensor = sub_image_tensor.cuda()
    
# #     with torch.no_grad():
# #         landmarks_pred = model['net'](sub_image_tensor)
    
# #     # Convertir la prédiction en CPU si nécessaire
# #     if use_gpu:
# #         landmarks_pred = landmarks_pred.cpu()
...
#             new_key = key[7:]  # Supprimer le préfixe "module."
...
#         print("end_voxels", end_voxels)
#         print("end_voxels", type(end_voxels))
#         # Utilisez une compréhension de liste pour diviser chaque coordonnée de voxel par 3
#         # resultats = [[valeur/3 for valeur in coordonnees] for coordonnees in end_voxels]
#         # end_voxels=[[80,80,80]]
...
#         # Convertir la liste de tenseurs en un seul tenseur si nécessaire
#         for idx, landmark_pred in enumerate(voi_landmark_mask_preds):
#              print(f"Element {idx}: {type(landmark_pred)}")
 
#         landmark_mask_preds = voi_landmark_mask_preds[0]  # Utilisez la première prédiction comme exemple

#         # Concaténer les tenseurs dans la liste voi_landmark_mask_preds
#         landmark_mask_preds = torch.cat(voi_landmark_mask_preds, dim=0)
...
def detection(input_path, model_folder, gpu_id, return_landmark_file, save_landmark_file, save_prob, output_folder):
    ...
    for i, file_path in enumerate(file_path_list):
        ...
        for j in range(0, model['num_landmark_classes']):
          ...
          # only keep probability of the largest connected component
          landmark_mask_cc = sitk.GetArrayFromImage(landmark_mask_cc)
          masked_landmark_mask_prob = np.multiply(landmark_mask_cc.astype(float), landmark_mask_prob)
